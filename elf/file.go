...
func (f *File) applyRelocations(dst []byte, rels []byte) error {
	...
	case f.Class == ELFCLASS64 && f.Machine == EM_BPF:
		return f.applyRelocationsBPF(dst, rels)
	default:
...
// R_BPF The BPF ELF reloc types for BPF.
...
const (
	...
	R_BPF_64_64 R_BPF = 1
	// R_BPF_64_32 indicates that 64 bits should be relocated
	R_BPF_64_32 R_BPF = 10
)

func (f *File) applyRelocationsBPF(dst []byte, rels []byte) error {
	return nil

	// 16 is the size of Rel64.
	if len(rels)%16 != 0 {
		return errors.New("length of relocation section is not a multiple of 24")
	}
	...
	for b.Len() > 0 {
		...
		switch t {
		case R_BPF_64_64:
			if rel.Off+8 >= uint64(len(dst)) {
				continue
			}
			val64 := sym.Value
			f.ByteOrder.PutUint64(dst[rel.Off:rel.Off+8], val64)
		case R_BPF_64_32:
